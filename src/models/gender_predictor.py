...
class GenderPredictor:
    ...
    def predict(self, image: np.ndarray) -> Tuple[str, float]:
        ...
        Returns:
            ...
            - confidence is max(output, 1-output), range [0.5, 1.0]
            - Sigmoid output < 0.5 → "Female", >= 0.5 → "Male"
        ...
        # Sigmoid threshold 0.5: output < 0.5 → "Female", output >= 0.5 → "Male"
        # Confidence = max(output, 1 - output)
        if prediction < 0.5:
            gender_label = "Female"
    ...
    def load(self, weights_path: str) -> None:
        ...
        try:
            ...
            try:
                ...
                # Backward compatibility for older weight-only artifacts.
                if self.model is None:
                    self._build_model()
    ...
    def save(self, weights_path: str) -> None:
        ...
        # Save the full model so .keras artifacts can be loaded directly.
